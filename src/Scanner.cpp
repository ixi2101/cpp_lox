...
bool Scanner::matchNext(char expected, std::vector<char>::iterator &iter,
...
void Scanner::skipAhead(char e, std::string::iterator& iter) {
    // copy the iter
    ...
    --lookForward;
    std::advance(iter, std::distance(iter, lookForward));
    // iter = lookForward - 1;
};
...
std::vector<Token> Scanner::scanTokens() {
  ...
  for (auto iter = m_src.begin(); iter != end;) {
    ...
    static auto float_literal = [&](TokenType type, float literal,
                                    std::string lex) {
